...
import { PublisherError, fail } from '../src/errors.js';
import { applyMarkdownRewrites } from '../src/markdown.js';
import { canonicalJson, checksum, stableId } from '../src/models.js';
import { attachmentSourceFileId, buildAttachmentBundle, readAttachmentSource } from '../src/attachment-bundle.js';
import { parseAttachmentLimit } from '../src/config.js';
...
export async function publishPlan(plan, { adapter, root = process.cwd(), bundleOptions, adapterFactory } = {}) {
  // Validation emits v2 only. Do not route its revision contract through the
  // legacy reconciler before private bundle verification/promotion is complete.
  if (plan.version === 2 || plan.materials.some((material) => 'attachmentsRevision' in material || 'bundle' in material)) fail('PUBLICATION_PROTOCOL_PENDING', 'Revision-aware publication is not enabled; this baseline is local preparation only');
  if (!adapter) fail('FIXTURE_ADAPTER_REQUIRED', 'Legacy regression fixtures require an explicit in-memory adapter');
  return withCourseLock(plan.course.slug, async () => {
    let stage = 'prepare-local-files';
    try {
      const prepared = await preparePublicationFiles(plan, root, bundleOptions);
      stage = 'preflight';
