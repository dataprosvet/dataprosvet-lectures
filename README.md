...
Markdown: до 0,25 MiB. Изображения: только PNG/JPEG, до 5 MiB, 4096×4096 и 16 млн пикселей; WebP не принимается. Внешние изображения и raw HTML запрещены.
...
- Готовый общий ZIP без сжатия — до **≈28,61 MiB (точно 30 000 000 байт)**, включая headers/central directory. Превышение отклоняет весь план до загрузок и любых удалённых изменений, без обрезания, разбиения или частичной публикации. **10 × 10 MiB не помещаются**: автор сам пересматривает состав или размеры файлов.
