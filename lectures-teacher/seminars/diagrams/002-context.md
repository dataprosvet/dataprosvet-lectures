```mermaid
%% Native Mermaid C4. Declaration order and row size control placement.
%% Relationships describe the initiator of an interaction; responses are implicit.
%% BankOffices supplies office metadata and visit history (assumption for this reference).
%%{init: {"c4": {"diagramMarginX": 40, "diagramMarginY": 40, "width": 260, "height": 110, "c4ShapeMargin": 70, "c4ShapePadding": 20}, "wrap": true}}%%
C4Context
  title Уровень 1 · Контекст ClientFlowPredictor
  Person(client, "User", "Клиент: смотрит прогноз потока")
  System(predictor, "ClientFlowPredictor", "Система прогнозирования потока клиентов")
  System_Ext(email, "EmailServer", "Доставляет письма с предупреждениями")
  Person(manager, "OfficeManager", "Руководитель офиса")
  ...
  Rel(client, predictor, "Смотрит прогноз потока")
  Rel(predictor, email, "Передаёт письма<br/>для доставки")
  Rel(manager, managerSystem, "Смотрит нагрузку и прогноз")
  Rel(managerSystem, predictor, "Запрашивает готовые данные")
  Rel(predictor, offices, "Запрашивает офисы, штат и посещения")
  UpdateLayoutConfig($c4ShapeInRow="3", $c4BoundaryInRow="1")
  UpdateRelStyle(client, predictor, $offsetX="-60", $offsetY="-30")
  UpdateRelStyle(predictor, email, $offsetX="-60", $offsetY="-30")
  UpdateRelStyle(manager, managerSystem, $offsetX="-60", $offsetY="-30")
  UpdateRelStyle(managerSystem, predictor, $offsetX="-180", $offsetY="0")
  UpdateRelStyle(predictor, offices, $offsetX="35", $offsetY="0")
...
%% Light fills keep labels fully opaque and readable.
  ...
  UpdateElementStyle(manager, $bgColor="#edf4fa", $fontColor="#25364a", $borderColor="#9bb5ca")
  UpdateElementStyle(predictor, $bgColor="#dbeafe", $fontColor="#1e3a5f", $borderColor="#93b4d7")
