...
const root='/tmp/lecture02-full',asset='/Users/ripper/vaults/Teaching/Tim/c2s3/api-technologies/assets/lecture-002/';
...
function notes(n,extra){let b=content[n-1]?.notes??'';let refs=[...b.matchAll(/\[\^([^\]]+)\]/g)].map(m=>m[1]);let defs=foot.split('\n').filter(l=>refs.some(r=>l.startsWith('[^'+r+']:'))).join('\n');return b+'\n'+defs+'\n'+(extra??'');}
...
function code(topic,sub,n,listing,explanation,extra=''){let s=slide(topic,sub,n,extra);box(s,70,214,750,420);text(s,listing,88,228,716,389,23,C.ink,false,mono);text(s,explanation,862,240,344,370,27);return s;}
...
bullets('Рекап','Что берём в сегодняшнюю работу',2,[['Проектируем от потребителя','Получение готового прогноза для руководителя офиса.'],['Описываем договор','Операции, данные, результаты, ошибки и ограничения.'],['Проверяем результат','HTTP → REST → форматы → схемы → OpenAPI.']]);
...
code('HTTP','Получение готового прогноза',4,'GET /offices/42/forecast?days=1 HTTP/1.1\nHost: 127.0.0.1:8000\nAccept: application/json','GET — намерение\n\n42 — office_id в path\n\ndays=1 — query\n\nAccept — желаемый формат ответа');
s=slide('HTTP','Передача наблюдения: заполненный запрос',4);await image(s,'http11-observation.png',66,205,850,442);text(s,'Content-Type',942,265,270, forty(),27,C.accent,true);text(s,'Формат тела\nэтого сообщения',942,313,270,100,25);text(s,'30 байт',942,460,270,45,29,C.accent,true);text(s,'Длина JSON\nбез перевода строки',942,508,270,100,25);
...
bullets('HTTP','Интерактив: сервер обработал запрос, ответ потерялся',13,[['GET / PUT','Прогноз / полное состояние threshold=100, notifications_enabled=true.'],['POST','Добавление нового наблюдения.'],['PATCH','«Установить 100» или «прибавить 10»: что даст повтор?']]);
const statuses=[
[14,'1xx: промежуточный ответ','cats-1xx.png',[['100','Можно продолжить отправку тела']]],
[15,'2xx: что именно удалось','cats-2xx.png',[['200','Успешный результат'],['201','Создан ресурс'],['202','Принято; обработка ещё не завершена'],['204','Выполнено; тела ответа нет']]],
...
bullets('REST','Связи и доступные действия',19,[['Гипермедиа','Клиент узнаёт связи и действия из представления.'],['Пример проектирования','links.office.href = "/offices/42"'],['Границы','Одной ссылки недостаточно для REST. Code-on-demand необязателен.']]);
...
code('Форматы данных','XML: документ для партнёра',22,listing(22,'xml'),'application/xml\n\nЭлементы, атрибуты, пространства имён\n\nXSD = XML Schema Definition\n\nСхема XML: элементы, атрибуты и типы');
...
}`,'properties ≠ required\n\nnull ≠ отсутствие ≠ 0\n\nПроектируемая схема. Мок null не принимает.');
for(let stage=1;stage<=3;stage++){s=slide('Схемы и валидация','Проверки до выполнения действия',26,'Кадр '+stage+' из 3; все кадры занимают 2 минуты суммарно.');const xs=[['JSON-парсер','{"visitors": }','Не разобрать сообщение'],['Схема','{"visitors": -5}','Нарушено minimum: 0'],['Предметные правила','{"office_id": 999}','Офис неизвестен']];for(let i=0;i<stage;i++){let y=221+i*128;box(s,75,y,72,72);text(s,'0'+(i+1),86,y+12,60,44,31,C.accent,true);text(s,xs[i][0],182,y,1010,44,29,C.ink,true);text(s,xs[i][1],182,y+50,411,40,25,C.muted);text(s,'→',593,y+46,56, forty(),29,C.accent);text(s,xs[i][2],666,y+49,532,44,25);}if(stage===3)text(s,'Допустимый запрос → действие → проверка исходящего ответа',182,620,1020,40,24,C.accent,true);}
rows('Схемы и валидация','Тип сохранён, смысл изменился',27,[['Было','visitors: 45 — посетителей за час'],['Стало','visitors: 45 — посетителей за сутки'],['Последствие','Схема проходит; клиент считает неправильно']]);
...
bullets('OpenAPI','Тот же документ в Swagger UI',30,[['Открываем /lecture-02','Страница читает авторский /lecture-02/openapi.yaml.'],['Находим соответствия','Parameters ↔ parameters; Responses ↔ responses.'],['Различаем','Example Value — пример; Schema — ограничения.\nПример ещё не является ответом сервера.']]);
...
const res=await finalizePresentation({workspaceDir:root,candidatePath:root+'/build/candidate.pptx',finalPath:root+'/output/002_http-rest-openapi-data-contracts-final.pptx',pythonExecutable:'/Users/ripper/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3',integrityValidatorPath:skill+'/container_tools/inspect_presentation_package_integrity.py',layoutValidatorPath:skill+'/container_tools/inspect_presentation_layout_geometry.py',layoutArgs:['--expected-slide-size-emu','12192000,6858000','--validate-heading-fit'],fontPolicy:{basis:'design',families:[font,mono]},explicitTotalSlideCount:map.length,verifyArtifactToolImport:true,receiptPath:root+'/build/validation-final.json'});console.log(JSON.stringify({slides:map.length,result:res}));
