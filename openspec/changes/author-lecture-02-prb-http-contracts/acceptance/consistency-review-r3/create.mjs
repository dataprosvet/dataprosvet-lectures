...
const root='/tmp/lecture02-r3',asset='/Users/ripper/vaults/Teaching/Tim/c2s3/api-technologies/assets/lecture-002/';
...
for(let stage=1;stage<=3;stage++){s=slide('Схемы и валидация','Проверки до выполнения действия',26,'Кадр '+stage+' из 3; все кадры занимают 2 минуты суммарно.');const xs=[['JSON-парсер','{"visitors": }','Не разобрать сообщение'],['Схема','{"office_id":42,"visitors":-5}','Нарушено minimum: 0'],['Предметные правила','{"office_id":999,"visitors":45}','Офис неизвестен']];for(let i=0;i<stage;i++){let y=221+i*128;box(s,75,y,72,72);text(s,'0'+(i+1),86,y+12,60,44,31,C.accent,true);text(s,xs[i][0],182,y,1010,44,29,C.ink,true);highlight(text(s,xs[i][1],182,y+50,480,40,20,'#000000',false,mono),xs[i][1]);text(s,'→',670,y+46,56, forty(),29,C.accent);text(s,xs[i][2],750,y+49,450,44,24);}if(stage===3)text(s,'Допустимый запрос → действие → проверка исходящего ответа',182,620,1020,40,24,C.accent,true);}
...
rows('OpenAPI','Проверка GET: Swagger UI и curl',31,[['42 / days=1','Swagger UI → 200 · 24 часовые точки'],['42 / days=7','Swagger UI → 200 · 168 часовых точек'],['42 / days=2','curl → 422 · invalid_request']]);
...
const overrides={
51:'В Swagger UI выполнить GET для офиса 42 с days=1 и days=7: 24 и 168 часовых точек. Для days=2 выполнить curl -i http://127.0.0.1:8000/offices/42/forecast?days=2 (URL заключить в кавычки): ожидать 422 и error.code=invalid_request. Enum в UI допускает только 1/7. Не путать Example Value с Server response.',
...
for(let n=1;n<=37;n++){
 ...
 body=body.replace(/(\d{1,2}):(\d{2})–(\d{1,2}):(\d{2})/g,(m,a,b,c,d)=>fmt(start+60*Number(a)+Number(b))+'–'+fmt(start+60*Number(c)+Number(d)));
...
const res=await finalizePresentation({workspaceDir:root,candidatePath:root+'/build/candidate.pptx',finalPath:root+'/output/002_http-rest-openapi-data-contracts-r3.pptx',pythonExecutable:'/Users/ripper/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3',integrityValidatorPath:skill+'/container_tools/inspect_presentation_package_integrity.py',layoutValidatorPath:skill+'/container_tools/inspect_presentation_layout_geometry.py',layoutArgs:['--expected-slide-size-emu','12192000,6858000','--validate-heading-fit'],fontPolicy:{basis:'design',families:[font,mono]},explicitTotalSlideCount:map.length,verifyArtifactToolImport:true,receiptPath:root+'/build/validation.json'});console.log(JSON.stringify({slides:map.length,result:res}));
