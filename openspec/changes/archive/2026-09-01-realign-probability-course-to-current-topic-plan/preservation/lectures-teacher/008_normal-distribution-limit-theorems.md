...
|---|---|---|
| Основной учебник: предельные теоремы | [В. Е. Гмурман. «Теория вероятностей и математическая статистика»: глава 9, §§ 2–6, PDF-с. 102–111](<../sources/Gmurman_V_E_Teoria_veroyatnostey_i_matematicheskaya_statistika.pdf#page=102>) | неравенство Чебышёва, закон больших чисел Чебышёва и теорема Бернулли |
| Основной учебник: нормальный закон | [В. Е. Гмурман: глава 12, §§ 2–8, PDF-с. 128–138](<../sources/Gmurman_V_E_Teoria_veroyatnostey_i_matematicheskaya_statistika.pdf#page=128>) | нормальное распределение, стандартизация, интервальные вероятности, правило трёх сигм и ЦПТ |
| Дополнительный источник | [Дайзенрот, Фейзал, Онг. «Математика в машинном обучении»: § 6.5, PDF-с. 254–263](<../sources/математика для машинного обучения.pdf#page=254>) | гауссово распределение и линейные преобразования |
| Прикладной контекст | [Николенко, Кадурин, Архангельская. «Глубокое обучение»: глава 2, § 2.1, PDF-с. 40–53](<../sources/Glubokoe_obuchenie_Pogruzhenie_v_mir_neyronnykh_setey.pdf#page=40>) | статистический контекст, вероятностные модели и осторожная интерпретация вывода |
| Нормативная основа | [РПД: нормальный закон, предельные теоремы, задачи и вопросы, PDF-с. 11–20](<../sources/Б1.О.09.01 Теория вероятностей РПД СИИ ПРБ.pdf#page=11>) | обязательные темы, самостоятельная работа и компетенции |

Подробная библиографическая карта с печатными страницами и ограничениями использования приведена в конце лекции.

## Блок 1. Переход к нормальной модели (0–7 минут)
...
|---|---|
| ОПК-1.1 | Студент выбирает нормальную или предельную модель и формулирует её предпосылки. |
| ОПК-1.2 | Студент стандартизует величины и различает точную вероятность, границу, приближение и асимптотику. |
| ОПК-1.3 | Студент интерпретирует вероятностный результат и базовую терминологию проверки гипотез. |
| ОПК-6.1 | Студент строит вычислительный маршрут нормальной задачи и предельного рассуждения. |
| ОПК-6.2 | Студент сопоставляет теоретические результаты с воспроизводимой симуляцией. |
| ОПК-6.3 | Студент диагностирует нарушение условий независимости, одинакового распределения и конечности дисперсии. |
| MF-1.2 | Студент применяет нормальную модель и предельные теоремы к анализу данных. |
| MF-1.3 | Студент аргументирует выбор вероятностного вывода и его ограничения. |

### Входные знания

Функция распределения $F$, плотность $f$, математическое ожидание $E[X]$, дисперсия $\operatorname{Var}(X)$, стандартное отклонение $\sigma(X)$, стандартизация и интегральная функция Лапласа. В этой лекции рабочее обозначение $\Phi$ означает полную стандартную нормальную функцию распределения $P(Z\le z)$; если в таблице дана функция Лапласа $L(z)=\Phi(z)-1/2$, сначала выполняется этот перевод.
...
## Блок 2. Нормальное распределение и параметры (7–24 минуты)
...
## Блок 3. Стандартизация и интервалы (24–40 минут)
...
## Блок 4. Задачи ИДЗ и контрольной работы (40–50 минут)
...
видно, что $\mu=-2$, $\sigma=3$, $\operatorname{Var}(X)=9$.
...
$$
\operatorname{Var}(3E[X]-2X)=(-2)^2\operatorname{Var}(X)=36.
$$

Так как $E[X^2]=\operatorname{Var}(X)+(E[X])^2=9+4=13$,

$$
E[3X^2-5]=3\cdot13-5=34.
$$
...
## Блок 5. Неравенство Чебышёва (50–61 минута)

### 8. Неравенство Чебышёва

Для любой величины с конечными $E[X]$ и $\operatorname{Var}(X)$:
...
$$
P(|X-E[X]|<\varepsilon)
\ge1-\frac{\operatorname{Var}(X)}{\varepsilon^2}.
$$
...
**Сопоставимый числовой пример.** Пусть известно только $E[X]=20$ и $\sigma(X)=2$. Интервал $(14;26)$ имеет вид $|X-20|<6$, то есть $\varepsilon=3\sigma$. Без знания закона Чебышёв гарантирует лишь
...
Обозначим событие большого отклонения через

$$
A=\{|X-E[X]|\ge\varepsilon\}.
$$

Разобьём ожидание неотрицательной величины $(X-E[X])^2$ по событиям $A$ и $\overline A$:

$$
\operatorname{Var}(X)
=E[(X-E[X])^2\mathbf 1_A]
+E[(X-E[X])^2\mathbf 1_{\overline A}].
$$

Второе слагаемое неотрицательно, поэтому его можно отбросить при получении нижней границы:

$$
\operatorname{Var}(X)\ge E[(X-E[X])^2\mathbf 1_A].
$$

На событии $A$ квадрат отклонения не меньше $\varepsilon^2$, то есть
$(X-E[X])^2\mathbf 1_A\ge\varepsilon^2\mathbf 1_A$. Монотонность ожидания даёт

$$
\operatorname{Var}(X)
\ge\varepsilon^2E[\mathbf 1_A]
=\varepsilon^2P(A).
$$

Так как $\varepsilon^2>0$, делим обе части и возвращаем определение $A$:
...
Доказательство не использовало форму распределения; нужны только существование конечной дисперсии и положительное $\varepsilon$.

## Блок 6. Законы больших чисел и ЦПТ (61–73 минуты)
...
$$
\operatorname{Var}(X_k)\le C\quad\text{для всех }k.
$$
...
Обозначим центрированное среднее через

$$
Y_n=\frac1n\sum_{k=1}^n(X_k-E[X_k]).
$$

Сначала $E[Y_n]=0$ по линейности ожидания. Сдвиг на постоянную не меняет дисперсию, а независимость устраняет все ковариационные слагаемые. Поэтому
...
где последняя оценка следует из $\operatorname{Var}(X_k)\le C$ для каждого $k$. Теперь для любого фиксированного $\varepsilon>0$ применяем неравенство Чебышёва:

$$
P(|Y_n|\ge\varepsilon)
\le\frac{\operatorname{Var}(Y_n)}{\varepsilon^2}
\le\frac{C}{n\varepsilon^2}\longrightarrow0.
$$

Следовательно,
$P(|Y_n|<\varepsilon)=1-P(|Y_n|\ge\varepsilon)\to1$, что по определению означает $Y_n\xrightarrow{P}0$.
...
## Блок 7. Режимы вывода и визуальная проверка (73–82 минуты)
...
| Оценка Чебышёва | Известны только конечные $E[X]$ и $\operatorname{Var}(X)$ | Универсальная верхняя или нижняя граница | «не больше / не меньше» |
...
$$
E[\overline X_n]=0{,}3,\qquad
\operatorname{Var}(\overline X_n)=\frac{0{,}21}{n}.
$$
...
## Блок 8. Введение в проверку гипотез (82–88 минут)
...
## Блок 9. Ошибки и завершение курса (88–90 минут)
...
1. Для $E[X]=20$, $\sigma=2$ получить оценку Чебышёва для $(14;26)$ и объяснить, почему это не точная вероятность.
...
[Семинар 16](../seminars/116_normal-distribution-intervals.md) · [Семинар 17](../seminars/117_control-work-3.md) · [Python-код лекции](../attachments/lecture_8.py)
