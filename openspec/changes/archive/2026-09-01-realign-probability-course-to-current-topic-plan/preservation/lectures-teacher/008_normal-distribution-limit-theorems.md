...
Идея доказательства проста. На событии $|X-E[X]|\ge\varepsilon$ квадрат отклонения не меньше $\varepsilon^2$. Поэтому средний квадрат отклонения не меньше произведения $\varepsilon^2$ на вероятность этого события:

$$
\operatorname{Var}(X)\ge\varepsilon^2P(|X-E[X]|\ge\varepsilon).
$$
...
$$
\operatorname{Var}\!\left(\frac1n\sum_{k=1}^nX_k\right)
