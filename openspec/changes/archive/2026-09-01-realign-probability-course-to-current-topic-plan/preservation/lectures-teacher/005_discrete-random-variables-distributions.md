...
|---|---|---|
| Основной учебник | [В. Е. Гмурман. «Теория вероятностей и математическая статистика»: глава 6, §§ 1–6, PDF-с. 65–70](<../sources/Gmurman_V_E_Teoria_veroyatnostey_i_matematicheskaya_statistika.pdf#page=65>) | случайная величина, дискретные законы, биномиальное и пуассоновское распределения |
| Основной учебник: CDF | [В. Е. Гмурман: глава 10, §§ 1–4, PDF-с. 112–116](<../sources/Gmurman_V_E_Teoria_veroyatnostey_i_matematicheskaya_statistika.pdf#page=112>) | функция распределения, её свойства и исходное соглашение $P(X<x)$ |
| Дополнительный источник | [Дайзенрот, Фейзал, Онг. «Математика в машинном обучении»: §§ 6.1.2 и 6.2, PDF-с. 228–238](<../sources/математика для машинного обучения.pdf#page=228>) | случайная величина как отображение, дискретные и непрерывные распределения |
| Прикладной контекст | [Николенко, Кадурин, Архангельская. «Глубокое обучение»: глава 2, § 2.1, PDF-с. 40–43](<../sources/Glubokoe_obuchenie_Pogruzhenie_v_mir_neyronnykh_setey.pdf#page=40>) | вероятностное описание данных и моделей |
| Нормативная основа | [РПД: темы 3–4 и кейс № 2, PDF-с. 11–14](<../sources/Б1.О.09.01 Теория вероятностей РПД СИИ ПРБ.pdf#page=11>) | обязательные распределения, задания и фиксация дефекта исходных данных |

Подробная библиографическая карта с печатными страницами и ограничениями использования приведена в конце лекции.

## Блок 1. От числа успехов к распределению (0–7 минут)
...
|---|---|
| ОПК-1.1 | Студент переводит исходы эксперимента в случайную величину и выбирает форму задания распределения. |
| ОПК-1.2 | Студент проверяет нормировку, границы функции распределения и условия биномиальной или пуассоновской модели. |
| ОПК-1.3 | Студент интерпретирует распределение и вероятности событий в прикладном контексте. |
| ОПК-6.1 | Студент строит ряд, многоугольник и функцию распределения по единому алгоритму. |
| ОПК-6.2 | Студент использует вычислительную проверку и визуализацию распределения. |
| ОПК-6.3 | Студент обнаруживает противоречивые исходные данные и не подменяет их выдуманными значениями. |
| MF-1.2 | Студент применяет дискретные распределения к моделям числа событий в данных. |
...
## Блок 2. Случайная величина и её тип (7–20 минут)
...
## Блок 3. Ряд и многоугольник распределения (20–39 минут)
...
## Блок 4. Функция распределения и соглашения (39–57 минут)
...
Если $x_1< x_2$, то каждый исход, для которого $X\le x_1$, автоматически удовлетворяет и $X\le x_2$. Поэтому

$$
\{X\le x_1\}\subseteq\{X\le x_2\}.
$$

Разность $\{X\le x_2\}\setminus\{X\le x_1\}$ не пересекается с первым событием, и по аддитивности

$$
F_X(x_2)=F_X(x_1)+P(x_1<X\le x_2)\ge F_X(x_1).
$$

Это доказывает монотонность без ссылки на рисунок.
...
Для дискретной величины между соседними возможными значениями набор исходов $\{X\le x\}$ не меняется. Поэтому функция постоянна на промежутках и изменяется только скачками. В самой точке $x_k$ принято условие $X\le x_k$, поэтому масса $P(X=x_k)$ уже включена в $F_X(x_k)$. Справа от $x_k$ до следующего возможного значения новых исходов нет, значит значение сохраняется: это и есть правосторонняя непрерывность ступенчатой функции. Слева масса точки ещё не включена, поэтому

$$
F_X(x_k)-F_X(x_k-0)=P(X=x_k).
$$
...
## Блок 5. Биномиальная модель и исходные данные (57–70 минут)
...
## Блок 6. Пуассоновская модель (70–81 минута)
...
Подпись перед иллюстративной таблицей РПД указывает $p=0{,}3$, однако сами вероятности таблицы соответствуют $p=0{,}1$. Это дефект подписи, а не другое численное решение. Расчёт по тексту задачи выполняется при $p=0{,}1$:
...
Любая аналитическая работа начинается с проверки согласованности текста, параметров и таблицы. Здесь ошибочную подпись $p=0{,}3$ отклоняем, потому что условие и весь числовой ряд согласованы с $p=0{,}1$.
...
$$
E[X]=0{,}5,\qquad \operatorname{Var}(X)=0{,}45,\qquad \sigma(X)\approx0{,}6708.
$$
...
## Блок 7. Переходы между формами и завершение (81–90 минут)
...
[Семинар 10](../seminars/110_discrete-distribution-row-polygon.md) · [Python-код лекции](../attachments/lecture_5.py)
